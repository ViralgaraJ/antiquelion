...
class Search extends DAO {
      ...
      foreach($city as $c) {
        ...
        if($c!='') {
          if(is_numeric($c)) {
            ...
            $esc_c = $this->dao->escapeStr($c);
            $this->cities[] = sprintf("(%st_item_location.s_city LIKE '%%%s%%' OR %st_item_location.s_city_native LIKE '%%%s%%' OR %st_item_location.s_city_area LIKE '%%%s%%' OR %st_item_location.s_address LIKE '%%%s%%') ", DB_TABLE_PREFIX, $esc_c, DB_TABLE_PREFIX, $esc_c, DB_TABLE_PREFIX, $esc_c, DB_TABLE_PREFIX, $esc_c);
          }
        }
      }
    } else {
    ...
    if(is_array($region)) {
      foreach($region as $r) {
        ...
        if($r != '') {
          if(is_numeric($r)) {
            ...
            $esc_r = $this->dao->escapeStr($r);
            $this->regions[] = sprintf("(%st_item_location.s_region LIKE '%%%s%%' OR %st_item_location.s_region_native LIKE '%%%s%%') ", DB_TABLE_PREFIX, $esc_r, DB_TABLE_PREFIX, $esc_r);
          }
        }
      }
      ...
      if($region != '') {
        if(is_numeric($region)) {
          ...
          $esc_r = $this->dao->escapeStr($region);
          $this->regions[] = sprintf("(%st_item_location.s_region LIKE '%%%s%%' OR %st_item_location.s_region_native LIKE '%%%s%%') ", DB_TABLE_PREFIX, $esc_r, DB_TABLE_PREFIX, $esc_r);
        }
      }
    }
  ...
  private function _makeSQLPremium($num = 2, $rand = false) {
    ...
    if($this->withPattern) {
      ...
      $this->dao->join(DB_TABLE_PREFIX . 't_item_location as l', 'l.fk_i_item_id = ti.pk_i_id', 'LEFT');
      $this->dao->where('ti.pk_i_id = d.fk_i_item_id');

      $search_pattern_cond = '';
      
      if($this->sPattern != '') {
        if(osc_search_pattern_method() == 'ft') {
          ...
          $search_pattern_cond = sprintf("lower(concat(coalesce(d.s_title,''), ' ', coalesce(d.s_description,''), ' ', coalesce(l.s_city,''), ' ', coalesce(l.s_city_native,''), ' ', coalesce(l.s_region,''), ' ', coalesce(l.s_region_native,''), ' ', coalesce(l.s_city_area,''), ' ', coalesce(l.s_address,''))) like '%%%s%%'", strtolower(trim((string)$this->sPattern)));
        }
      }
  ...
  private function _makeSQL($count = false) {
    ...
    // Join tables
    if($this->withLocations || $this->withPattern || OC_ADMIN) {
      $this->dao->join(sprintf('%st_item_location as l', DB_TABLE_PREFIX), sprintf('l.fk_i_item_id = %st_item.pk_i_id', DB_TABLE_PREFIX, DB_TABLE_PREFIX), 'LEFT');
      $this->_addLocations();
    }
