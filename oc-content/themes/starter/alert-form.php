...
<div id="n-block" class="block <?php if(osc_is_web_user_logged_in()) { ?>logged_user<?php } else { ?>unlogged_user<?php } ?>">
  <div class="n-wrap">
    <h3><i class="fa fa-bell-o"></i> <span><?php _e('Subscribe to Search Alerts', 'starter'); ?></span></h3>
    <div class="text"><?php _e('Get new listings matching this search criteria sent instantly to your inbox.', 'starter'); ?></div>
    ...
    <form action="<?php echo osc_base_url(true); ?>" method="post" name="sub_alert" id="sub_alert">
      ...
      <?php if(osc_is_web_user_logged_in()) { ?>
        ...
        <button type="button" class="btn alert-notify-btn alert-notify"><i class="fa fa-envelope-o"></i> <span><?php _e('Subscribe to this Search', 'starter'); ?></span></button>
      ...
      <?php } else { ?>
        <div class="n-box">
          ...
          <button type="button" class="btn alert-notify-btn alert-notify"><span><?php _e('Subscribe', 'starter'); ?></span></button>
        </div>
      <?php } ?>
    </form>
  </div>
